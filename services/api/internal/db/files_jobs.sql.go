...
const listPendingPDFJobs = `-- name: ListPendingPDFJobs :many
SELECT id, tenant_id, template_code, payload, status, file_id, error_message, created_at, updated_at FROM pdf_jobs
WHERE status = 'pending'
ORDER BY created_at ASC
LIMIT $1
`

func (q *Queries) ListPendingPDFJobs(ctx context.Context, limit int32) ([]PdfJob, error) {
	rows, err := q.db.Query(ctx, listPendingPDFJobs, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
